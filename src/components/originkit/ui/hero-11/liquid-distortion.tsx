...
`;
    ...
    // Pause rendering when offscreen to save CPU/GPU. Use IntersectionObserver
    // to restart when the element re-enters the viewport.
    const observer = new IntersectionObserver(
      (entries) => {
        const e = entries[0];
        if (e && e.isIntersecting) {
          if (!rafRef.current) render(0);
        } else {
          if (rafRef.current) {
    ...
    // If already visible on mount, start rendering immediately.
